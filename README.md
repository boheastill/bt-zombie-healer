...
- But the **BLE acceptance path is dead**: a BLE keyboard/mouse disconnects for its normal power nap (`0x13 Remote User Terminated`) and never gets accepted again — during the failure the host-side capture (btmon) shows scanning + IRK resolving list armed and running, yet **zero advertisement callbacks and all `connect` attempts time out**, while the same peripheral's radio is demonstrably healthy (pairing-mode capture at −58 dBm). Whether it keeps transmitting directed advertisements mid-failure is a strong inference not yet confirmed by a second receiver — the smoking-gun direct observation is `kernel: hci0: ACL packet for unknown connection handle`: a **live connection dropped by the host mid-use**, peripheral data arriving for a handle the host forgot. `connect` times out forever.
